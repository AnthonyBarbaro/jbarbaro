...
import { getAggregateRating, getLocations, getLocationsPageContent } from "@/lib/cms";
import { buildGoogleMapsEmbedUrl } from "@/lib/maps";
import { buildMetadata } from "@/lib/seo";
import { formatPhone } from "@/lib/utils";

export async function generateMetadata() {
  const content = await getLocationsPageContent();
...
export default async function LocationsPage() {
  const [content, locations, aggregateRating] = await Promise.all([
        ...
        <Container>
          <Card className="bg-ink text-ivory">
            <CardContent>
              ...
              >
                {content.closingBadge}
              </Badge>
              <h2 className="mt-4 font-heading text-3xl sm:text-4xl">{content.closingTitle}</h2>
              <p className="mt-3 max-w-2xl text-sm leading-7 text-ivory/82">{content.closingDescription}</p>
              <div className="mt-6 flex flex-wrap gap-3">
                {content.closingButtons.map((button, index) => (
                  <ButtonLink
