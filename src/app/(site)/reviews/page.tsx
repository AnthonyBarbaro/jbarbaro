...
import { getReviewsPageContent, getSiteSettings, getTestimonials } from "@/lib/cms";
...
export default async function ReviewsPage() {
  const [content, siteSettings, testimonials] = await Promise.all([
    ...
    <>
      ...
      <WaveSection topWave="B" bottomWave="C" background="ivory">
        <Container>
          <Card tone="stone">
            <CardContent>
              <p className="text-sm font-semibold tracking-[0.14em] text-deep-teal uppercase">{content.aggregateLabel}</p>
              <h2 className="mt-3 font-heading text-5xl text-ink sm:text-6xl">
                {siteSettings.ratingValue} <span className="text-xl text-smoke sm:text-2xl">/ 5.0</span>
              </h2>
              <p className="mt-2 text-sm text-smoke">Based on {siteSettings.reviewCount} customer reviews</p>
