...
import { getAggregateRating, getLocationMap, getLocations } from "@/lib/cms";
...
export default async function LocationDetailPage({ params }: { params: Promise<{ locationSlug: string }> }) {
  const { locationSlug } = await params;
  const [locationMap, aggregateRating] = await Promise.all([getLocationMap(), getAggregateRating()]);
  const location = locationMap[locationSlug];
