...
import { getBrands, getCategories, getCategoryMap } from "@/lib/cms";
...
export default async function MenCategoryPage({ params }: { params: Promise<{ categorySlug: string }> }) {
  const { categorySlug } = await params;
  const [categoryMap, categories, brands] = await Promise.all([getCategoryMap(), getCategories(), getBrands()]);
  const category = categoryMap[categorySlug];
