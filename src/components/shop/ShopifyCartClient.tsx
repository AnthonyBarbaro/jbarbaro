...
function getSelectedOptionValue(options: ShopifyCartSnapshot["lines"][number]["selectedOptions"], optionName: string) {
  const normalizedOptionName = optionName.toLowerCase();

  return options.find((option) => option.name.toLowerCase().includes(normalizedOptionName))?.value ?? null;
}

function optionValue(options: ShopifyCartSnapshot["lines"][number]["selectedOptions"], optionName: string) {
...
function getSizeChoices(cartLine: ShopifyCartSnapshot["lines"][number]) {
  ...
  const nonSizeOptions = cartLine.selectedOptions.filter((option) => !option.name.toLowerCase().includes("size"));
  ...
  return Array.from(choices.values()).sort((left, right) => left.label.localeCompare(right.label, undefined, { numeric: true }));
}
...
export function ShopifyCartClient() {
  ...
  async function updateLineVariant(cartLine: ShopifyCartSnapshot["lines"][number], merchandiseId: string) {
                        ...
                        <div className="mt-4 max-w-[15rem]">
                          ...
                          >
                            {sizeChoices.map((choice) => (
                              <option key={choice.id} value={choice.id} disabled={!choice.availableForSale && !choice.isCurrent}>
                                {choice.label}
                          ...
                          <p className="mt-2 text-xs leading-5 text-smoke">
                            {isLineMutating ? "Updating size..." : "Switch sizes here without rebuilding your bag."}
                          </p>
                        </div>
                      ) : currentSize ? (
          ...
          <Card>
            <CardContent>
              ...
              <div className="mt-6 space-y-3">
                ...
                <Link
                  href="/shop"
                  className="inline-flex min-h-11 w-full items-center justify-center rounded-md border border-ink/20 px-5 py-2.5 text-xs font-semibold tracking-[0.14em] text-ink uppercase transition-colors hover:border-gold hover:text-gold"
                >
                  Continue Shopping
                </Link>
              </div>
            ...
            <CardContent>
              ...
              <div className="mt-5 space-y-4 text-sm leading-7 text-smoke">
                ...
                <div className="flex items-start gap-3">
                  ...
                  <p>Bag totals refresh live as you adjust quantities or switch an available size.</p>
                </div>
                <div className="flex items-start gap-3">
